...
var slice$=[].slice;(function(){var diff,sharedbWrapper;diff=function(o,n,dostr){dostr==null&&(dostr=true);return json0OtDiff(o,n,dostr?diffMatchPatch:null)};sharedbWrapper=function(opt){var p;opt==null&&(opt={});import$(this,{scheme:(opt.url||(opt.url={})).scheme||window.location.protocol.replace(":",""),domain:opt.url.domain||window.location.host,path:p=opt.url.path||opt.path||"/ws"});this.path=p[0]==="/"?p:"/"+p;this.scheme=this.scheme==="http"?"ws":"wss";this.collection=opt.collection||"doc";this.evtHandler={};this.reconnectInfo={retry:0,pending:[]};this.reconnect();return this};sharedbWrapper.prototype=import$(Object.create(Object.prototype),{json:{diff:function(o,n,dostr){dostr==null&&(dostr=true);return diff(o,n,dostr)}},getSnapshot:function(arg$){var id,version,this$=this;id=arg$.id,version=arg$.version;return new Promise(function(res,rej){return this$.connection.fetchSnapshot(this$.collection,id,version!=null?version:null,function(e,s){if(e){return rej(e)}else{return res(s)}})})},ready:function(){var this$=this;return new Promise(function(res,rej){if(this$.connected){return res()}if(!this$.reconnectInfo.handler){return this$.reconnect()}return this$.reconnectInfo.pending.push({res:res,rej:rej})})},get:function(arg$){var id,watch,create,this$=this;id=arg$.id,watch=arg$.watch,create=arg$.create;return(!this.connection?this.reconnect():Promise.resolve()).then(function(){return new Promise(function(res,rej){var doc;doc=this$.connection.get(this$.collection,id);return doc.fetch(function(e){if(e){return rej(e)}doc.subscribe(function(ops,source){return res(doc)});doc.on("error",function(err){return this$.fire("error",{doc:doc,err:err})});if(watch!=null){doc.on("op",function(ops,source){return watch(ops,source)})}if(!doc.type){return doc.create((create?create():null)||{})}})})})},on:function(n,cb){var ref$;return((ref$=this.evtHandler)[n]||(ref$[n]=[])).push(cb)},fire:function(n){var v,i$,ref$,len$,cb,results$=[];v=slice$.call(arguments,1);for(i$=0,len$=(ref$=this.evtHandler[n]||[]).length;i$<len$;++i$){cb=ref$[i$];results$.push(cb.apply(this,v))}return results$},disconnect:function(){if(!this.socket){return}this.socket.close();this.socket=null;this.connected=false;return this.socket=null},reconnect:function(){var this$=this;return new Promise(function(res,rej){var delay;if(this$.socket){return res()}delay=this$.reconnectInfo.retry++;delay=Math.round(Math.pow(delay,1.4)*500);clearTimeout(this$.reconnectInfo.handler);console.log("try reconnecting ("+this$.reconnectInfo.retry+") after "+delay+"ms ...");return this$.reconnectInfo.handler=setTimeout(function(){this$.reconnectInfo.handler=null;this$.socket=new WebSocket(this$.scheme+"://"+this$.domain+this$.path);this$.connection=new sharedb.Connection(this$.socket);this$.socket.addEventListener("close",function(){this$.socket=null;this$.connected=false;return this$.fire("close")});return this$.socket.addEventListener("open",function(){var ref$;this$.reconnectInfo.retry=0;((ref$=this$.reconnectInfo).pending||(ref$.pending=[])).splice(0).map(function(it){return it.res()});this$.connected=true;return res()})},delay)})}});if(typeof module!="undefined"&&module!==null){module.exports=sharedbWrapper}if(typeof window!="undefined"&&window!==null){return window.sharedbWrapper=sharedbWrapper}})();function import$(obj,src){var own={}.hasOwnProperty;for(var key in src)if(own.call(src,key))obj[key]=src[key];return obj}
var sdbHost,sdbAdapter,slice$=[].slice;sdbHost=function(opt){opt==null&&(opt={});this.opt=opt;this.evtHandler={};this.adapters=[];return this};sdbHost.prototype=import$(Object.create(Object.prototype),{on:function(n,cb){var ref$;return((ref$=this.evtHandler)[n]||(ref$[n]=[])).push(cb)},fire:function(n){var v,i$,ref$,len$,cb,results$=[];v=slice$.call(arguments,1);for(i$=0,len$=(ref$=this.evtHandler[n]||[]).length;i$<len$;++i$){cb=ref$[i$];results$.push(cb.apply(this,v))}return results$},getdoc:function(){var ref$,ref1$,this$=this;return this.sdb.get((ref1$={watch:function(o,s){return this$.watch(o,s)}},ref1$.id=(ref$=this.opt).id,ref1$.create=ref$.create,ref1$)).then(function(doc){return this$.doc=doc})},watch:function(ops,source){if(this.opt.watch){this.opt.watch(ops,source)}return this.adapters.map(function(it){return it.opsIn({ops:ops,source:source})})},reconnect:function(it){var this$=this;this.fire("reconnecting",it);return this.sdb.reconnect().then(function(){return this$.getdoc()}).then(function(){return this$.adapters.map(function(a){return a.setHost(this$)})}).then(function(){if(this$.opt.reconnect){return this$.opt.reconnect()}}).then(function(it){return this$.fire("reconnected",it)})},initSdb:function(){var sdb,this$=this;this.fire("init-sdb");this.sdb=sdb=new sharedbWrapper({url:this.opt.url});sdb.on("error",function(it){return this$.fire("error",it)});if(!(this.opt.autoReconnect!=null)||this.opt.autoReconnect){sdb.on("close",function(){return this$.reconnect()})}return this.reconnect().then(function(){return sdb.ready()})},adapt:function(opt){var a,ref$;opt==null&&(opt={});this.adapters.push(a=new sdbAdapter((ref$=import$({},opt),ref$.host=this,ref$)));return a}});sdbAdapter=function(opt){opt==null&&(opt={});this.opt=opt;this.data={};this.evtHandler={};this.path=opt.path;this.setHost(opt.host);this.setPath(opt.path);return this};sdbAdapter.prototype=import$(Object.create(Object.prototype),{on:function(n,cb){var ref$;return((ref$=this.evtHandler)[n]||(ref$[n]=[])).push(cb)},off:function(n){return this.evtHandler[n]=[]},fire:function(n){var v,i$,ref$,len$,cb,results$=[];v=slice$.call(arguments,1);for(i$=0,len$=(ref$=this.evtHandler[n]||[]).length;i$<len$;++i$){cb=ref$[i$];results$.push(cb.apply(this,v))}return results$},setHost:function(host){(this.sdb=host.sdb,this.doc=host.doc,this).host=host;return this.updateData()},setPath:function(p){this.path=p;return this.updateData()},updateData:function(){var o,i$,ref$,len$,n;if(!this.doc){return}o=this.doc.data;for(i$=0,len$=(ref$=this.path||[]).length;i$<len$;++i$){n=ref$[i$];o=o[n]||{}}return this.opsIn({data:o})},opsIn:function(arg$){var ops,data,source,o,i$,ref$,len$,n,this$=this;ops=arg$.ops,data=arg$.data,source=arg$.source;if(data){this.data=data}else{o=this.doc.data;for(i$=0,len$=(ref$=this.path).length;i$<len$;++i$){n=ref$[i$];o=o[n]||{}}this.data=o}if(ops){ops=ops.filter(function(op){var i$,to$,i;for(i$=0,to$=this$.path.length;i$<to$;++i$){i=i$;if(op.p[i]!==this$.path[i]){return 0}}return 1}).map(function(op){op=import$({},op);op.p=op.p.slice(this$.path.length);return op})}if(!ops||ops.length){return this.fire("ops-in",{ops:ops,data:this.data,source:source})}},opsOut:function(ops){var cur,o,p,opsAddon,i$,ref$,len$,n,ref1$,this$=this;if(!this.sdb||!this.doc.submitOp){return}if(typeof ops==="function"){cur=ops(this.data);ops=this.sdb.json.diff(this.data||{},cur);return this.opsOut(ops)}else if(Array.isArray(ops)&&ops.length){ops.map(function(it){return it.p=this$.path.concat(it.p)});o=this.doc.data;p=[];opsAddon=[];for(i$=0,len$=(ref$=this.path).length;i$<len$;++i$){n=ref$[i$];p.push(n);if(!o[n]){opsAddon.push({p:JSON.parse(JSON.stringify(p)),oi:n===(ref1$=this.path)[ref1$.length-1]&&Array.isArray(this.data)?[]:{}})}o=o[n]||{}}ops=opsAddon.concat(ops);return this.doc.submitOp(ops)}}});sdbAdapter["interface"]={update:function(){},opsIn:function(arg$){var ops,data,source;ops=arg$.ops,data=arg$.data,source=arg$.source;if(ops){this.data=this.adapter.host.doc.type.apply(this.data,ops)}else{this.data=JSON.parse(JSON.stringify(data))}return this.update()},opsOut:function(f){return this.adapter.opsOut(f)},adapt:function(arg$){var host,path,a,this$=this;host=arg$.host,path=arg$.path;if(a=this.adapter){a.setHost(host);a.setPath(path)}else{this.adapter=a=host.adapt({path:path})}this.adapter.off("ops-in");this.adapter.on("ops-in",function(opt){return this$.opsIn(opt)});return this.opsIn({data:this.adapter.data})},setPath:function(it){return this.adapter.setPath(it)},adapted:function(){return!!this.adapter}};function import$(obj,src){var own={}.hasOwnProperty;for(var key in src)if(own.call(src,key))obj[key]=src[key];return obj}
